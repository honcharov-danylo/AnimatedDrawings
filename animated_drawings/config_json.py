...
class Config:
    def __init__(self, cfg:Dict, animated_characters: List[Dict["character_cfg":character_cfg,
                                                           "retarget_cfg":retarget_cfg, "motion_cfg":motion_cfg]]):
        """
        Initializes base config.
        :param cfg: dictionary of parameters, as in json_configs_example/example_config.json
        :param animated_characters: List of dictionaries (character_cfg, retarget_cfg, motion_cfg), where:
            - character_cfg: dictionary of parameters, as in json_configs_example/character.json
            - motion_cfg: dictionary of parameters, as in json_configs_example/motion.json
            - retarget_cfg: dictionary of parameters, as in json_configs_example/retarget.json
        """
        self.view: ViewConfig = ViewConfig(cfg['view'])
        self.scene: SceneConfig = SceneConfig(cfg['scene'], animated_characters)
        self.controller: ControllerConfig = ControllerConfig(cfg['controller'])
        ...
        if self.controller.mode == 'video_render' and self.controller.output_video_path is not None and self.controller.output_video_path.endswith(
                '.mp4'):
            try:
    ...
    #     :param cfg: dictionary with all parameters, necessary for the config.
    #     Expected to have next keys: "view", "scene", "controller", with nexted dictionaries as values.
    #     :return: new Config object
    #     """
    #     return cls(cfg['view'], cfg['scene'], cfg['controller'])

class SceneConfig:
    def __init__(self, scene_cfg: dict, animated_characters_list:List) -> None:
        ...
        each: Dict[str, str]
        for each in animated_characters_list:
            char_cfg: Dict = each['character_cfg']
            motion_cfg: Dict = each['motion_cfg']
            retarget_cfg: Dict = each['retarget_cfg']
            self.animated_characters.append((
                CharacterConfig(char_cfg),
                RetargetConfig(retarget_cfg),
...
class CharacterConfig:
    ...
    def __init__(self, char_cfg: Dict) -> None:  # noqa: C901
        ...
        try:
            self.mask_p: Path = resolve_ad_filepath(char_cfg['mask_path'],'mask_cfg')
            self.txtr_p: Path = resolve_ad_filepath(char_cfg['texture_path'],'texture_cfg') # second param shoudln't matter
...
class MotionConfig:
    def __init__(self, motion_cfg: Dict) -> None:  # noqa: C901
...
class RetargetConfig:
    ...
    def __init__(self, retarget_cfg: Dict) -> None:  # noqa: C901
        ...
        # fixing mapping from json

        fixed_mapping  = dict()

        for k,v in retarget_cfg['char_joint_bvh_joints_mapping'].items():
            fixed_mapping[k] = tuple(v)

        try:
            self.char_joint_bvh_joints_mapping = fixed_mapping #retarget_cfg['char_joint_bvh_joints_mapping']
